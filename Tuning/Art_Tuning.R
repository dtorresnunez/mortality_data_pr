#Corridas para artículo de investigación
...
carpeta_resultados <- file.path(script_dir, "resultados_modelos/ArtTuning")
...
df$region_int <- df$region_idx
df$period_int <- df$period_idx
    ...
    guardar    = TRUE)
{
  ...
  nombre_modelo <- paste(
    ...
    "s_t", model_reg_per, model_per_reg, par_p_s_t, par_q_s_t, par_b_s_t,
    #"cel", model_cel, par_p_cel, par_q_cel, par_b_cel,
    "sam", nsamples,
    sep = "_"
  )
  
# Formula del modelo de ambos sexos
  formula_sb2 <- deaths ~
    factor(sex)+
    ...
    f(region_period_idx, model = model_reg_per, graph = Amat, group = period_idx, constr = TRUE,
      hyper = list(
        theta1 = list(prior = SB2.prior(par_p_s_t, par_q_s_t, par_b_s_t)),
        theta2 = list(prior = "logitbeta", param = c(0.5, 0.5))),
      control.group = list(model = model_per_reg,
  ...
  # --- Índices para efectos de interacción (deben ser columnas propias) ------
  # región × periodo
  
  
  # región × edad (si la vas a usar)
  df$regage <- df$region_idx
  df$agereg <- df$age_idx
  formula_sb2 <- deaths ~
    factor(sex) +
    f(age_idx, model = model_age, constr = TRUE,
      hyper = list(theta = list(prior = SB2.prior(par_p_age, par_q_age, par_b_age)))) +
    ...
    f(region_idx, model = model_reg, graph = g, constr = TRUE,
      hyper = list(theta = list(prior = SB2.prior(par_p_reg, par_q_reg, par_b_reg)),
                   phi   = list(prior = "logitbeta", param = c(0.5, 0.5)))) +
    f(region_int, model = model_reg, graph = Amat, group = period_int, constr = TRUE,
      hyper = list(
        theta1 = list(prior = SB2.prior(par_p_s_t, par_q_s_t, par_b_s_t)),
        theta2 = list(prior = "logitbeta", param = c(0.5, 0.5))),
      control.group = list(model = model_per,
                           hyper = list(theta = list(prior = SB2.prior(par_p_s_t, par_q_s_t, par_b_s_t)))))
...
#Formula del modelo para hombres o mujeres
  formula_h <- deaths ~
    f(age_idx, model = model_age, constr = TRUE,
...
#Resultado de muestras para la familia Poisson
...
#Indice de celda parámetros iniciales
# model_cel  = "iid",
# par_p_cel  = 1,
# par_q_cel  = 1,
# par_b_cel  = 0.05*0.05,
...
#1-10: 1,1,0.05
#11-21
# age = c(0.5, 0.5, 10/1.6),
# region = c(0.5, 0.5, 10/151),
# period = c(0.5, 0.5, 10/319)

# factor(sex) + RW2(age) + RW2(period)
# factor(sex) + RW2(age) + RW2(period) + BYM2(region)
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. region/periodo
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. periodo/edad
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. region/edad
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. region/periodo + int. region/edad
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. region/periodo + int. periodo/edad
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. periodo/edad + int. region/edad
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region) + int. region/periodo + int. region/edad + int. edad/periodo
# factor(sex)  + RW2(age) + RW2(period) + BYM2(region)  +cell_idx
# age    = c(0.5, 0.5, 10/1.6), 
# region = c(0.5, 0.5, 10/151), 
# period = c(0.5, 0.5, 10/319)
# f(region_int, model="bym2", graph=Amat,
#   group=period_int,
#   control.group=list(model="rw2")) +
#   f(regage, model="bym2", graph=Amat,
#     group=agereg,
#     control.group=list(model="rw2"))
# 
# https://rpubs.com/enwuliu/1444036
#https://teams.public.onecdn.static.microsoft/evergreen-assets/safelinks/2/atp-safelinks.html

#f(region_int, model="bym2", graph=Amat,group=period_int, control.group=list(model="rw2")) +



formula_sb2_v3 <- deaths ~
  factor(sex) +
  
  # Interacción Tipo IV: región × periodo  →  un solo triplete: par_p_st, par_q_st, par_b_st
  f(region_int, model = "bym2", graph = Amat, group = period_int,
    constr = TRUE,
    hyper = list(
      theta1 = list(prior = SB2.prior(par_p_st, par_q_st, par_b_st)),
      theta2 = list(prior = "logitbeta", param = c(0.5, 0.5))
    ),
    control.group = list(
      model = "rw2",
      hyper = list(theta = list(prior = SB2.prior(par_p_st, par_q_st, par_b_st)))
    )) +
  
  # Interacción Tipo IV: región × edad  →  un solo triplete: par_p_ra, par_q_ra, par_b_ra
  f(regage, model = "bym2", graph = Amat, group = agereg,
    constr = TRUE,
    hyper = list(
      theta1 = list(prior = SB2.prior(par_p_ra, par_q_ra, par_b_ra)),
      theta2 = list(prior = "logitbeta", param = c(0.5, 0.5))
    ),
    control.group = list(
      model = "rw2",
      hyper = list(theta = list(prior = SB2.prior(par_p_ra, par_q_ra, par_b_ra)))
    )) +
  
  # Interacción Tipo IV: periodo × edad  →  un solo triplete: par_p_pa, par_q_pa, par_b_pa
  f(period_age_idx, model = "rw2", group = age_group_idx,
    constr = TRUE,
    hyper = list(theta = list(prior = SB2.prior(par_p_pa, par_q_pa, par_b_pa))),
    control.group = list(
      model = "rw2",
      hyper = list(theta = list(prior = SB2.prior(par_p_pa, par_q_pa, par_b_pa)))
    ))